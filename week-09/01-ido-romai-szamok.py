#Adott két dátum romai szamokkal és a két dátum közti eltérést kell kiszámolni napokban.
import re, datetime 
...
def datumma_alakit(romai_datum):
    romai_ev, romai_honap, romai_nap = romai_datum.split('-')
    return datetime.date(arabszammakonvertal(romai_ev), arabszammakonvertal(romai_honap), arabszammakonvertal(romai_nap))

def elteres(romai_datum1, romai_datum2):
    return romai_konvertal((datumma_alakit(romai_datum1) - datumma_alakit(romai_datum2)).days)

print(elteres('MMXXII-IV-IV','MMXXI-IV-IV'))