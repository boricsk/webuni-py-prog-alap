...
class SzovegesKerdes(Kerdes):
    ...
    def __init__(self, kerdes, **kwargs) -> None:
        super().__init__(kerdes, kwargs['helyes_valasz'])
        self.minta = kwargs['minta']
...
class KerdesekType1:
    ...
    def __iter__(self):
        self.kerdesek_es_valaszok = [
            SzovegesKerdes('Itt volt Magyarország első egyeteme. :',helyes_valasz = 'Pécs', minta = r'^P[eé]cs$'),
            SzovegesKerdes('Itt gyóntak az árpád házi királyok. :',helyes_valasz = 'Báta', minta = r'^B[aá]ta$'),
            SzovegesKerdes('1687 Augusztusában itt mértek döntő vereséget a török seregre. :', helyes_valasz = 'Nagyharsány', minta = r'^Nagyhars[aá]ny$'),
            SzovegesKerdes('Melyik várost viseli a "Koronázóváros" címet? :',helyes_valasz = 'Székesfehérvár', minta = r'^Sz[eé]kesfeh[eé]rv[aá]r$'),
            SzovegesKerdes('Mi ausztria fővárosa? :',helyes_valasz = 'Bécs', minta = r'^(B[eé]cs|Wien)$'),
            SzovegesKerdes('Ki játszotta a Bodie-t és a Doyle-t a The Professionals-ban? :',helyes_valasz = 'Lewis Collins és Martin Shaw', minta = r'^(Lewis Collins( és |,| |, | ,){1}Martin Shaw)$'),
            SzovegesKerdes('Paul Hunn milyen rekordot állított fel, amelynél 118.1 decibelt regisztráltak? ',helyes_valasz = 'A leghangosabb böfögés',minta = r'^[Aa] leghangosabb (b[oö]f[oö]g[eé]s|b[uü]fiz[eé]s|b[oö]ffent[eé]s){1}$'),
...
match mode:
    ...
    case _ : print('Nincs ilyen lehetőség!')
