...
dataFile = './week-11/exam/europa-tavai.json'
felsorolDataFile = './week-11/exam/felsorol-kviz.json'
topScoreDataFile = './week-11/exam/kviz.dat'
datumKvizDataFileKerdes = './week-11/exam/datum-kerdes.kviz'
datumKvizDataFileValasz = './week-11/exam/datum-valasz.kviz'
NormalDataFile = './week-11/exam/normal-kviz.json'
...
class DatumKviz():
    def datum_kviz():
        ...
        datum_pont, datum_kerdesek_szama, jo_valaszok_szama, valasz, topScoreDate = 0, 0, 0, '', TopScore.topScoreRead()
        start = datetime.now()
        print('A válaszokat EEEE-HH-NN kérem!')
        
        try:
            with open(datumKvizDataFileKerdes, encoding='utf-8') as datum_kerdesek_file:
        ...
        try:
            with open(datumKvizDataFileValasz, encoding='utf-8') as datum_valasz_file:
        ...
        if datum_pont > int(topScoreDate[3]):
            ...
            TopScore.topScoreWrite(topScoreDate[0], topScoreDate[1],topScoreDate[2], datum_pont)

class TippelosKviz():
    def tippelos_kviz():
        tipp_pont, jo_valaszok, tipp_valaszlehetoseg, tipp_kerdesek_szama, TopScoreFelsorol = 0, 0, 4, 5, TopScore.topScoreRead()    
        global dataFile
        if FileCheck.checkFileLocation(dataFile):
            with open(dataFile, encoding='utf-8') as TippelosFile:
        ...
        if tipp_pont > int(TopScoreFelsorol[2]):
            print('Gratulálunk! Rekordot döntöttél!')
            TopScore.topScoreWrite(TopScoreFelsorol[0], TopScoreFelsorol[1], tipp_pont, TopScoreFelsorol[3])

class FelsorolKviz():
    def felsorol_kviz():
            global felsorolDataFile
            fels_pont, fels_beirt_valasz, talalatok = 0, set(), set()
            TopScoreFelsorol = TopScore.topScoreRead()
            
            if FileCheck.checkFileLocation(felsorolDataFile):
            ...
            for felsKvizAdatok in felsorol_kerdesek_es_valaszok_json:
                ...
                if fels_pont > int(TopScoreFelsorol[1]):
                    print('Gratulálunk! Rekordot döntöttél!')
                    TopScore.topScoreWrite(TopScoreFelsorol[0], fels_pont, TopScoreFelsorol[2],TopScoreFelsorol[3])
                fels_beirt_valasz.clear()
                talalatok.clear()

class TopScore():
    global topScoreDataFile
    def topScoreWrite(normal, felsorol, tipp, datum):
        TopScores = [normal, felsorol, tipp, datum]
        TopScoreFile = open(topScoreDataFile,mode='w')
    ...
    def topScoreRead():
        TopScores = []
        TopScoreFile = open(topScoreDataFile,mode='r')
...
def kerdest_feltesz(kerdes, **kwargs):
    while True:
        valasz = input(kerdes)
        match kwargs['tipus']:
            case 'SZOVEG':
    ...
    match kwargs['tipus']:
        case 'SZOVEG':
            ...
            #if valasz == kwargs['helyes_valasz']:
                pont += 5
        ...
        case 'EGESZ_SZAM':
            ...
            if valasz_szam == kwargs['helyes_valasz']:
                pont += 5
                return True, 'A valasz helyes'
            else:
                elteres = abs(valasz_szam - kwargs['helyes_valasz']) / kwargs['helyes_valasz'] * 100
            match elteres:
                case n if 0 <= n <= 2:
                    pont += 4
        ...
        case 'TIZEDES_SZAM':
            tolerancia = 0.05
            #valasz_float = float(valasz)
            if kwargs['helyes_valasz'] - tolerancia < valasz_float < kwargs['helyes_valasz'] + tolerancia:
                pont += 5
                return True, 'A válsaz helyes!'
            else:
                return False, 'A válasz helytelen! A helyes válasz ' + str(kwargs['helyes_valasz']) + ' lett volna.'

        
def main():
    global helyes_valaszok, kerdesek_szama, NormalDataFile
    if os.name == 'nt':
        os.system('cls')
    else:
        os.system('clear')
    ...
    match mode:
        case '1':
            ...
            if FileCheck.checkFileLocation(NormalDataFile):
            ...
            for normalKvizAdatok in kerdesek_es_valaszok_json:
                kerdesek_szama +=1
                helyes_e, valasz_szöveg = kerdest_feltesz(normalKvizAdatok['kerdes'], helyes_valasz = normalKvizAdatok['valasz'], tipus = normalKvizAdatok['KerdesTipus'], minta = normalKvizAdatok['regex'])
                print(valasz_szöveg)
                if helyes_e:
                    helyes_valaszok +=1
            ...
            if pont > int(TopScoreNormal[0]):
                print('Gratulálunk, rekordot döntöttél!!')
                TopScore.topScoreWrite(pont, TopScoreNormal[1],TopScoreNormal[2],TopScoreNormal[3])
        
        case '2': FelsorolKviz.felsorol_kviz()
        case '3': TippelosKviz.tippelos_kviz()
        case '4': DatumKviz.datum_kviz()
